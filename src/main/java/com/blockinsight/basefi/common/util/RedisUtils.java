...
public class RedisUtils
{
     ...
     *
     * @param lockName   键
     * @param lockTimeout  时间(秒) time要大于0 如果time小于等于0 将设置无限期
     * @param acquireTimeout 连接时长
     * @return true成功 false 失败
     */
    public String setIfAbsent(String lockName, long acquireTimeout, long lockTimeout)
    {
    ...
    public boolean releaseDistributedLock(String lockName, String lockValue) {
        ...
        do {
            ...
            if (!Objects.equal(lockValue, redisTemplate.opsForValue().get(lockKey))) {
                break;
            }
    ...
    public boolean execute(String script, String key, String value, int expireTime)
    {
        RedisScript<String> redisScript = new DefaultRedisScript<>(script, String.class);
        Object result = redisTemplate.execute(redisScript, new StringRedisSerializer(), new StringRedisSerializer(), Collections.singletonList(key), value, String.valueOf(expireTime));
